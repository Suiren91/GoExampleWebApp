...
func TestJWTer_GetToken(t *testing.T) {
	...
	want, err := jwt.NewBuilder().JwtID(uuid.New().String()).Issuer(`github.com/Suiren91/GoExampleWebApp`).Subject("access_token").IssuedAt(c.Now()).Expiration(c.Now().Add(30*time.Minute)).Claim(RoleKey, "test").Claim(UserNameKey, "test_user").Build()
	if err != nil {
		t.Fatal(err)
	}
	...
	req := httptest.NewRequest(http.MethodGet, `https://github.com/Suiren91`, nil)
	...
	if !reflect.DeepEqual(got, want) {
		t.Errorf("GetToken() want: %v, got: %v", want, got)
	}
}
...
func TestJWTer_GetToken_NG(t *testing.T) {
	...
	c := clock.FixedClocker{}
	tok, err := jwt.NewBuilder().JwtID(uuid.New().String()).Issuer(`github.com/Suiren91/GoExampleWebApp`).
		Subject("access_token").IssuedAt(c.Now()).Expiration(c.Now().Add(30*time.Minute)).Claim(RoleKey, "test").
